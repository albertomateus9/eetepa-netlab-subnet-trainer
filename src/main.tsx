    ...
    <main>
      <section className="hero">
        ...
        <aside className="notice">
          ...
          <span>Educational portfolio prototype</span>
          <small>Not an official institutional system. Synthetic data only.</small>
        </aside>
      </section>
      {children}
    </main>
    ...
    <Shell>
      <div className="grid">
        <Panel title="CIDR Lab">
          ...
          <label>VLAN count {vlans}<input type="range" min="2" max="8" value={vlans} onChange={(event) => setVlans(Number(event.currentTarget.value))} /></label>
          <div className="metrics"><Metric label="Hosts per subnet" value={String(hosts)} /><Metric label="Subnet mask" value={'/' + prefix} /><Metric label="VLANs" value={String(vlans)} tone="ok" /></div>
          <button onClick={() => downloadText('eetepa-subnet-plan.md', report)}>Export Markdown</button>
        </Panel>
        <Panel title="Visual Address Plan">
...
function PacketPathVisualizer() {
  const steps = [
    { name: 'Client', detail: 'Student workstation prepares a request.', latency: 2 },
    ...
    { name: 'Response', detail: 'Packets return and the browser renders data.', latency: 52 },
  ];
    ...
    <Shell>
      <div className="grid">
        <Panel title="Protocol Timeline">
          ...
          <div className="metrics"><Metric label="Current layer" value={steps[active].name} /><Metric label="Synthetic latency" value={String(total) + ' ms'} tone="ok" /></div>
          <button onClick={() => downloadText('packet-path-report.md', report)}>Export Markdown</button>
        </Panel>
        <Panel title="Latency Budget">
...
function DataEtlPlayground() {
  ...
  const cleaned = useMemo<ExportRow[]>(() => {
    ...
    return rows.map((row): ExportRow => ({ ...row, status: row.status || 'needs-review' })).filter((row) => {
    ...
    <Shell>
      <div className="grid">
        <Panel title="Local Data Input">
          ...
          <div className="metrics"><Metric label="Raw rows" value={String(rows.length)} /><Metric label="Clean rows" value={String(cleaned.length)} tone="ok" /><Metric label="Ready" value={String(ready)} /></div>
        </Panel>
        <Panel title="Cleaned Table">
          ...
          <button onClick={() => downloadText('eetepa-clean-data.csv', toCsv(cleaned))}>Export CSV</button>
          <button onClick={() => downloadText('eetepa-clean-data.json', JSON.stringify(cleaned, null, 2))}>Export JSON</button>
        </Panel>
      </div>
    </Shell>
  );
    ...
    <Shell>
      <div className="grid">
        ...
        <Panel title="Latest Reading">
          {samples.slice(-1).map((sample) => <div className="metrics" key={sample.t}><Metric label="Temp" value={sample.temperature + ' C'} /><Metric label="Distance" value={sample.distance + ' cm'} /><Metric label="Light" value={sample.light + '%'} tone="ok" /></div>)}
          <p className="note">Serial data stays in the browser. Synthetic mode supports classes without hardware.</p>
          <button onClick={() => downloadText('maker-serial-samples.csv', toCsv(samples))}>Export CSV</button>
        </Panel>
      </div>
    </Shell>
  );
    ...
    <Shell>
      <div className="grid">
        <Panel title="Operations Metrics">
          ...
          <div className="metrics"><Metric label="Tickets" value={String(visible.length)} /><Metric label="SLA risk" value={String(breached)} tone={breached > 0 ? 'warn' : 'ok'} /><Metric label="Open" value={String(visible.filter((ticket) => ticket.status === 'open').length)} /></div>
        ...
        <Panel title="Ticket Queue">
          ...
          <button onClick={() => downloadText('helpdesk-analytics.csv', toCsv(visible))}>Export CSV</button>
        </Panel>
      </div>
    </Shell>
  );
...
function LabInventoryLite() {
  const [items, setItems] = useState([
    ...
    { asset: 'LAB-PC-02', room: 'Networks Lab', os: 'Windows', port: 'SW1-02', status: 'review' },
    ...
    <Shell>
      <div className="grid">
        <Panel title="Inventory Summary">
          <div className="metrics"><Metric label="Assets" value={String(items.length)} /><Metric label="Ready" value={String(items.filter((item) => item.status === 'ready').length)} tone="ok" /><Metric label="Review" value={String(items.filter((item) => item.status !== 'ready').length)} tone="warn" /></div>
          <button onClick={addAsset}>Add synthetic asset</button>
          <button onClick={() => downloadText('lab-inventory.json', JSON.stringify(items, null, 2))}>Export JSON</button>
          <button onClick={() => downloadText('lab-inventory.csv', toCsv(items))}>Export CSV</button>
        </Panel>
    ...
    <Shell>
      <div className="grid">
        <Panel title="Planning Controls">
          ...
          <button onClick={() => downloadText('schedule-plan.csv', toCsv(plan))}>Export CSV</button>
        </Panel>
    ...
    <Shell>
      <div className="grid">
        <Panel title="Awareness Lab">
          ...
          <button onClick={() => downloadText('cyber-range-report.json', JSON.stringify({ score, visible }, null, 2))}>Export JSON</button>
        </Panel>
        <Panel title="Synthetic Logs">
