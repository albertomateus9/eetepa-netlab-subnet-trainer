...
function PacketPathVisualizer() {
  const steps = [
    { name: 'Clienteeeeee', detail: 'Student workstation prepares a request.', latency: 2 },
...
function DataEtlPlayground() {
  ...
  const pronto = cleaned.filter((row) => row.status === 'pronto').length;
    ...
    <Shell>
      <div className="grid">
        <Panel title="Local Data Input">
          ...
          <div className="metrics"><Metric label="Raw rows" value={String(rows.length)} /><Metric label="Clean rows" value={String(cleaned.length)} tone="ok" /><Metric label="Pronto" value={String(pronto)} /></div>
        </Panel>
...
function LabInventoryLite() {
  const [items, setItems] = useState([
    { asset: 'LAB-PC-01', room: 'Maker Lab', os: 'Linux', port: 'SW1-01', status: 'pronto' },
    { asset: 'LAB-PC-02', room: 'Networks Lab', os: 'Windows', port: 'SW1-02', status: 'revisar' },
    { asset: 'KIT-IOT-01', room: 'Maker Lab', os: 'Firmware', port: 'USB', status: 'pronto' },
  ]);
  function addAsset() {
    ...
    <Shell>
      <div className="grid">
        <Panel title="Inventory Summary">
          <div className="metrics"><Metric label="Assets" value={String(items.length)} /><Metric label="Pronto" value={String(items.filter((item) => item.status === 'pronto').length)} tone="ok" /><Metric label="Review" value={String(items.filter((item) => item.status !== 'pronto').length)} tone="warn" /></div>
